...
## Debian slapd schema files: /etc/ldap/schema/ reference

Every schema shipped with Debian slapd 2.6, what it exists for and the attribute or
objectclass you will actually meet in the wild. Each ships as both .schema (slapd.conf
include format) and .ldif (OLC format). ppolicy is listed last: it ships no file at all
on 2.5+, the schema is compiled into the overlay module.

| File/Schema          | Used for                                                                                                                                                                                                                                                                                                              | Most frequently met field                                                             |
|----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| core.schema          | Mandatory foundation. Defines objectClass itself, the abstract top of the inheritance chain, the DN building blocks (cn, dc, o, ou, l, st, c) and the person/organizationalPerson chain plus groupOfNames. slapd does not start without it.                                                                           | `cn`: the RDN of most non-user entries and the naming attribute of half the directory |
| cosine.schema        | 1980s COSINE/X.500 pilot leftovers, RFC 4524. Supplies mail, manager, secretary, roomNumber, host, document* attributes plus the account and simpleSecurityObject objectclasses used for service accounts (your uid=authenticate entry).                                                                              | `mail`: the one COSINE attribute every deployment uses daily                          |
| inetorgperson.schema | RFC 2798, the de facto person schema of every corporate directory. Adds employeeNumber, departmentNumber, displayName, jpegPhoto, userSMIMECertificate, userPKCS12 on top of organizationalPerson. Your user entries are this.                                                                                        | `uid`: the RDN of essentially every user entry ever created                           |
| nis.schema           | RFC 2307, POSIX/Unix account mapping. posixAccount, posixGroup, shadowAccount plus uidNumber, gidNumber, loginShell, homeDirectory. The reason LDAP can back Linux logins via nsswitch/SSSD.                                                                                                                          | `uidNumber`: the field every PAM/NSS lookup resolves                                  |
| misc.schema          | Grab bag, mostly mail routing: inetLocalMailRecipient with mailLocalAddress, mailHost, mailRoutingAddress. Postfix/Sendmail LDAP routing tables live here. Explicitly marked experimental upstream.                                                                                                                   | `mailRoutingAddress`: the attribute MTAs query for delivery                           |
| collective.schema    | RFC 3671 collective attributes (c-l, c-o, c-PostalAddress and so on): values inherited by every entry in a subtree from a subentry. Requires the collect overlay. Your parking lot already holds a stance item on these.                                                                                              | `c-o`: shared organization name across a subtree, in the rare deployments using it    |
| corba.schema         | CORBA object references in the DIT (corbaObject, corbaIor). Dead technology, kept for compatibility. Your parking lot already sentences java/corba to denial in ldap4.                                                                                                                                                | `corbaIor`: stringified object reference, if you ever meet one                        |
| java.schema          | RFC 2713, serialized Java objects and JNDI references stored in the directory (javaSerializedObject, javaNamingReference). Same graveyard as corba.                                                                                                                                                                   | `javaClassName`: the FQCN tag on every such entry                                     |
| duaconf.schema       | Client (DUA) configuration profiles stored centrally: defaultServerList, defaultSearchBase, authenticationMethod. Solaris-era client autoconfiguration, mostly historical.                                                                                                                                            | `defaultSearchBase`: the one field a client profile always carries                    |
| dyngroup.schema      | Dynamic groups: groupOfURLs with memberURL holding an LDAP URL evaluated at read time by the dynlist overlay. Your original FEIDE dynlist plan lived here before back-ldap made it obsolete.                                                                                                                          | `memberURL`: the search-URL that defines membership                                   |
| dsee.schema          | Sun/Netscape Directory Server (DSEE) changelog compatibility: changeLogEntry, changeNumber, targetDn. For coexisting with or migrating from Sun-lineage servers.                                                                                                                                                      | `changeNumber`: the changelog sequence consumers poll                                 |
| msuser.schema        | Active Directory attribute/objectclass definitions (user, group, computer, sAMAccountName, userPrincipalName, unicodePwd, objectSid) so slapd can hold or proxy AD-shaped entries. Directly relevant to your back-ldap FEIDE proxy: these names are what rwm will be mapping.                                         | `sAMAccountName`: the AD login name every Windows auth path touches                   |
| openldap.schema      | OpenLDAP project vanity schema (OpenLDAPperson, OpenLDAPou). Effectively an example file, near zero production use.                                                                                                                                                                                                   | `displayName` via OpenLDAPdisplayableObject, the only piece anyone borrows            |
| namedobject.schema   | Minimal namedObject/namedPolicy structural classes for entries that need nothing but a name. Occasionally handy as a structural carrier for auxiliary-only data.                                                                                                                                                      | `cn` as bare RDN carrier                                                              |
| pmi.schema           | X.509 Privilege Management Infrastructure (ITU X.509 2000): attribute certificates, roles, privilege policies (pmiUser, role, attributeCertificateAttribute). Rare outside PKI-heavy shops.                                                                                                                           | `role`: the X.509 role attribute where PMI is deployed                                |
| ppolicy (module)     | Password policy enforcement schema: pwdPolicy objectclass plus pwd* policy attributes (pwdMaxFailure, pwdLockout, pwdInHistory) and per-user operational state (pwdFailureTime, pwdAccountLockedTime, pwdPolicySubentry). NO FILE on Debian 2.5+: compiled into the ppolicy overlay module, registered at moduleload. | `pwdPolicySubentry`: the per-user pointer that overrides the default policy           |
| openssh-lpk.schema   | THIRD PARTY, NOT SHIPPED with OpenLDAP: hand-installed custom schema for SSH public keys in the DIT. Defines sshPublicKey and the ldapPublicKey auxiliary objectclass; sshd fetches keys via AuthorizedKeysCommand helper. Never standardized by OpenSSH upstream; variants disagree on MUST/MAY. | `sshPublicKey`: the key blob sshd retrieves at login                                  |

### PMI, why rare

PMI is the authorization twin of PKI: attribute certificates bind privileges (roles,
clearances, permissions) to an identity, signed by an Attribute Authority, with their
own revocation machinery (ACRLs). The world went a different way. Real deployments do
authorization with LDAP group membership, AD security groups, Kerberos PACs and later
OAuth2 scopes and SAML assertions. All simpler, none requiring a second full
certificate infrastructure. PMI survives in defense, government ID schemes and some
